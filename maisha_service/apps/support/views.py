    ...
    def post(request):
        ...
        try:
            ...
            support_data = SupportDB(
                support_id=passed_data["support_id"],
                user_id=passed_data["user_id"],
                title=passed_data["title"],
                details=passed_data["details"],
                image=passed_data["image"],
                response=passed_data["response"],
                tracker=passed_data["tracker"]
            )
            support_data.save()
    ...
    def put(request):
        ...
        try:
            SupportDB.objects.filter(
                support_id=passed_data["support_id"]).update(
                        response=passed_data["response"],
                    )
            return Response({
                    "status": "success",
...
class SupportUserView(ListAPIView):
    """Get a user specific support"""
    serializer_class = SupportSerializer

    def get_queryset(self):
        return SupportDB.objects.filter(user_id=self.kwargs['user_id']).order_by('dateTime')
